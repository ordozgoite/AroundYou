...
enum AYEndpoints {
    ...
    case postNewPublication(text: String?, tag: String, imageUrl: String?, videoUrl: String?, videoThumbnailUrl: String?, latitude: Double, longitude: Double, isLocationVisible: Bool, token: String)
    case getPublicationCreationEligibility(token: String)
    ...
    case getAllPublicationsByUser(latitude: Double, longitude: Double, token: String)
    ...
    case checkNearByPublications(latitude: Double, longitude: Double, token: String)
    ...
    case postNewMessage(chatId: String, text: String?, imageUrl: String?, repliedMessageId: String?, clientMessageId: String?, token: String)
    case getMessages(chatId: String, timestamp: Int?, limit: Int, token: String)
    ...
    case postLostItem(lostItem: LostItem, token: String)
    case postReportIncident(reportIncident: ReportIncident, token: String)
    case deleteLostItem(lostItemId: String, token: String)
    case deleteReportIncident(reportId: String, token: String)
    ...
    case getMapPosts(minLat: Double, maxLat: Double, minLong: Double, maxLong: Double, token: String)
    case getAllPublicationsInRegion(bounds: MapClusterBounds, location: Location, token: String)
}
...
extension AYEndpoints: Endpoint {
    ...
    var path: String {
        ...
        case .getPublicationCreationEligibility:
            return "/api/Publication/GetPublicationCreationEligibility"
        ...
        case .postCommunityMessage:
            return "/api/CommunityMessage/PostCommunityMessage"
        case .getCommunityMessages:
        ...
        case .postLostItem:
            return "/api/LostItem/PostLostItem"
        case .postReportIncident:
            return "/api/ReportIncident/PostReport"
        case .deleteLostItem(let lostItemId, _):
            return "/api/LostItem/DeleteLostItem/\(lostItemId)"
        case .deleteReportIncident(let reportId, _):
            return "/api/ReportIncident/DeleteReport/\(reportId)"
        case .getReport(let reportId, _):
            return "/api/ReportIncident/GetReport/\(reportId)"
        case .getLostItem(let lostItemId, _):
        ...
        case .getMapPosts:
            return "/api/ExploreMap/GetMapPosts"
        case .getAllPublicationsInRegion:
            return "/api/Publication/GetAllPublicationsInRegion"
        }
    }
    ...
    var query: [String: Any]? {
        ...
        case .getAllPublicationsNearBy(let latitude, let longitude, _):
            ...
            return params
        case .checkNearByPublications(let latitude, let longitude, _):
            let params: [String: Any] = [
                "latitude": latitude,
                "longitude": longitude
            ]
        ...
        case .unlikePublication(let publicationId, _):
            ...
            return params
        case .getAllPublicationsByUser(let latitude, let longitude, _):
            let params: [String: Any] = [
                "latitude": latitude,
                "longitude": longitude
            ]
        ...
        case .unlikeComment(let commentId, let latitude, let longitude, _):
            ...
            return params
        case .getMessages(let chatId, let timestamp, let limit, _):
            var params: [String: Any] = [
                "chatId": chatId,
                "limit": limit
            ]
        ...
        case .getCommunitiesNearBy(let latitude, let longitude, _):
            ...
            return params
        case .getCommunityMessages(let communityId, let timestamp, _):
            var params: [String: Any] = [
        ...
        case .getCommunity(let communityId, let location, _):
            ...
            return params
        case .getMapPosts(let minLat, let maxLat, let minLong, let maxLong, _):
            let params: [String: Any] = [
                "maxLatitude": maxLat,
                "minLatitude": minLat,
            ...
            return params
        case .getAllPublicationsInRegion(let bounds, let location, _):
            let params: [String: Any] = [
    ...
    var header: [String : String]? {
        switch self {
        case .getPublicationCreationEligibility(let token):
            return [
                "Authorization": "Bearer \(token)",
                "Accept": "application/x-www-form-urlencoded",
                "Content-Type": "application/json"
            ]
        case .postNewPublication(_, _, _, _, _, _, _, _, let token), .getActivePublicationsNearBy(_, _, let token), .postNewUser(_, _, _, let token), .getUserInfo(_, _, let token), .likePublication(_, let token), .unlikePublication(_, let token), .getAllCommentsByPublication(_, let token), .postNewComment(_, _, _, let token), .deleteComment(_, let token), .deletePublication(_, let token), .getUserProfile(_, let token), .editProfile(_, let token), .getAllPublicationsByUser(_, _, let token), .postNewReport(_, let token), .postNewBugReport(_, let token), .blockUser(_, let token), .getBlockedUsers(let token), .unblockUser(_, let token), .likeComment(_, _, _, let token), .unlikeComment(_, _, _, let token), .getPublicationLikes(_, let token), .getCommentLikes(_, let token), .deleteProfilePic(let token), .getPublication(_, _, _, let token), .getUserNotifications(let token), .subscribeUserToPublication(_, let token), .unsubscribeUser(_, let token), .deleteNotification(_, let token), .deleteUser(let token), .getUserBanExpireDate(let token), .getAllPublicationsNearBy(_, _, let token), .postNewChat(_, let token), .getChatsByUser(let token), .muteChat(_, let token), .unmuteChat(_, let token), .postNewMessage(_, _, _, _, _, let token), .getMessages(_, _, _, let token), .deleteMessage(_, let token), .editPublication(_, _, _, _, _, _, let token), .finishPublication(_, let token), .deleteChat(_, let token), .getUnreadChatsNumber(let token), .verifyUserDiscoverability(let token), .activateUserDiscoverability(let token), .updateUserPreferences(_, _, _, _, _, _, let token), .deactivateUserDiscoverability(let token),.discoverUsersByPreferences(_, _, let token), .postNewCommunity(_, _, _, _, _, _, _, let token), .getCommunitiesNearBy(_, _, let token), .joinCommunity(_, _, _, let token), .askToJoinCommunity(_, _, _, let token), .getCommunityInfo(_, let token), .approveUserToCommunity(_, _, let token), .deleteCommunity(_, let token), .exitCommunity(_, let token), .removeUserFromCommunity(_, _, let token), .editCommunity(_, _, _, let token), .editCommunityDescription(_, _, let token), .postCommunityMessage(_, _, _, _, _, let token), .getCommunityMessages(_, _, let token), .deleteCommunityMessage(_, let token), .postNewBusiness(_, let token), .getBusinessesNearBy(_, let token), .deleteBusiness(_, let token), .getBusinessByUser(_, let token), .postLostItem(_, let token), .postReportIncident(_, let token), .deleteLostItem(_, let token), .deleteReportIncident(_, let token), .getReport(_, let token), .getLostItem(_, let token), .setItemAsFound(_, let token), .getMyCommunities(let token), .getRelevantCommunities(_, let token), .getCommunity(_, _, let token), .updateCommunityPrivacy(_, _, let token), .editBusiness(_, let token), .getMapPosts(_, _, _, _, let token), .getAllPublicationsInRegion(_, _, let token), .checkNearByPublications(_, _, let token):
            return [
                "Authorization": "Bearer \(token)",
                "Accept": "application/x-www-form-urlencoded",
                "Content-Type": "application/json"
    ...
    var body: [String : Any]? {
        ...
        case .getPublicationCreationEligibility:
            return nil
        case .postNewPublication(let text, let tag, let imageUrl, let videoUrl, let videoThumbnailUrl, let latitude, let longitude, let isLocationVisible, _):
            var params: [String: Any] = [
                "tag": tag,
                "latitude": latitude,
                "longitude": longitude,
            ...
            if let url = videoUrl { params["videoUrl"] = url }
            if let url = videoThumbnailUrl { params["videoThumbnailUrl"] = url }
            return params
        ...
        case .postNewChat(let otherUserUid, _):
            ...
            return params
        case .postNewMessage(let chatId, let text, let imageUrl, let repliedMessageId, let clientMessageId, _):
            var params: [String: Any] = [
                "chatId": chatId
            ]
            ...
            // Chave de idempotência: reenviar com ela devolve a mensagem já criada em vez de
            // criar uma segunda. É o que torna seguro tentar de novo horas depois.
        ...
        case .editCommunityDescription(let communityId, let description, _):
            ...
            return params
        case .postCommunityMessage(let communityId, let latitude, let longitude, let text, let repliedMessageId, _):
            var params: [String: Any] = [
                "communityId": communityId,
                "latitude": latitude,
                "longitude": longitude,
        ...
        case .postNewBusiness(let business, _):
            ...
            return params
        case .postLostItem(let lostItem, _):
            var params: [String: Any] = [
                "name": lostItem.name,
                "latitude": lostItem.latitude,
                "longitude": lostItem.longitude,
                "lostDate": lostItem.lostDate, 
                "hasReward": lostItem.hasReward
            ]
            
            if let description = lostItem.description { params["description"] = description }
            if let imageUrl = lostItem.imageUrl { params["imageUrl"] = imageUrl }
            if let locationDescription = lostItem.locationDescription { params["locationDescription"] = locationDescription }
            
            return params
        case .postReportIncident(let reportIncident, _):
            var params: [String: Any] = [
                "type": reportIncident.type.rawValue,
                "description": reportIncident.description,
                "latitude": reportIncident.latitude,
                "longitude": reportIncident.longitude
            ]
            
            if let imageUrl = reportIncident.imageUrl { params["imageUrl"] = imageUrl }
            if let isUserTheVictim = reportIncident.isUserTheVictim { params["isUserTheVictim"] = isUserTheVictim }
